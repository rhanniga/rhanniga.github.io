use crate::resume::{Education, Job, Resume, Skill};
use leptos::prelude::*;
...
pub fn Portfolio() -> impl IntoView {
    ...
    view! {
        <div class="retro-computer-window">
            <div class="window-titlebar">
                <div class="window-title">"Resume Window - WIP"</div>
        ...
        <header class="header">
            ...
            <div class="contact">
                <p>
                    ...
                    )>{contact_info.email.clone()}</a>
                    " | "
                    <a href=format!(
                        "tel:{}",
                        contact_info.phone.clone(),
                    )>{contact_info.phone.clone()}</a>
                </p>
                <p>
                    <a
                        href=format!("https://github.com/{}", contact_info.github.clone())
                        target="_blank"
                    >
                        "GitHub: "
                        {contact_info.github.clone()}
                    </a>
                    " | "
                    <a
                        href=format!("https://linkedin.com/in/{}", contact_info.linkedin.clone())
                        target="_blank"
                    >
                        "LinkedIn: "
                        {contact_info.linkedin.clone()}
                    </a>
                </p>
                <p>
...
fn Summary(summaries: Vec<crate::resume::Summary>) -> impl IntoView {
    view! {
        <section class="summary">
            ...
            {summaries
                .into_iter()
                .map(|summary| view! { <p>{summary.text}</p> })
                .collect::<Vec<_>>()}
        </section>
    }
}
                        ...
                        <div class="job">
                            ...
                            <ul class="bullets">
                                {job
                                    .bullets
                                    .into_iter()
                                    .map(|bullet| view! { <li>{bullet.text}</li> })
                                    .collect::<Vec<_>>()}
                            </ul>
                        </div>
                    }
...
fn Skills(skills: Vec<Skill>) -> impl IntoView {
    let expert_skills: Vec<_> = skills
        .iter()
        .filter(|s| s.experience == "expert")
        .cloned()
        .collect();
    let experienced_skills: Vec<_> = skills
        .iter()
        .filter(|s| s.experience == "experienced")
        .cloned()
        .collect();
    let skilled_skills: Vec<_> = skills
        .iter()
        .filter(|s| s.experience == "skilled")
        .cloned()
        .collect();
                ...
                view! {
                    <div class="skill-group">
                        ...
                        <div class="skill-list">
                            {expert_skills
                                .into_iter()
                                .map(|skill| view! { <span class="skill-tag">{skill.name}</span> })
                                .collect::<Vec<_>>()}
                        </div>
                    </div>
                }
